  ...
  return (
    <>
      {opencalls.map((item, i) => {
        ...
        return (
          <>
            {opencalls[i].attributes.date && differencePrev[i] != 0 && (
                ...
                >
                  <div className="image">
                    {item.attributes.cover_image.data && (
                      <Image
                        image={item.attributes.cover_image.data?.attributes}
                        layout="fill"
                        objectFit="cover"
                        sizes="50vw"
                      />
                  ...
                  <div className="info">
                    <div className="info-wrapper">
                                    ...
                                    >
                                      {date.single_date && (
                                        <>
                                          ...
                                          ,{" "}
                                          {Moment(date.single_date).format(
                                            "D MMM y"
                                          )}
                                        </>
                                      )}
                        ...
                        {item.attributes.tags?.data && (
                          <div className="tags">
                            {item.attributes.tags.data.map((tag, i) => {
                              return (
                                <a
                                  href={"/search/" + tag.attributes.slug}
                                  key={"search" + i}
                                >
                                  {tag.attributes.slug}
                                </a>
                      ...
                      {item.attributes.deadline && (
                        <div className="apply">
                          <img className="arrow" src="/arrow.svg" /> Apply until{" "}
                          {Moment(item.attributes.deadline).format("D MMM y")}
                        </div>
                      )}
                    </div>
                  </div>
