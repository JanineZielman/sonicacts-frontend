...
export async function getServerSideProps({params, query}) {
  ...
  const pageRes = 
    await fetchAPI( `/community-items?filters[biennials][slug][$ne]=biennial-2024&filters[slug][$eq]=${params.slug}${preview ? "&publicationState=preview" : '&publicationState=live'}&populate[content][populate]=*`
  );

  const pageRel = 
    await fetchAPI( `/community-items?filters[biennials][slug][$ne]=biennial-2024&filters[slug][$eq]=${params.slug}${preview ? "&publicationState=preview" : '&publicationState=live'}&populate=*`
  );
