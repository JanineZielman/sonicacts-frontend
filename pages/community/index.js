...
const Community = ({ menus, global, page, items, numberOfPosts }) => {
  ...
  const getMorePosts = async () => {
    const res = await fetchAPI(
      `/community-items?filters[biennials][slug][$ne]=biennial-2024&sort[0]=name&pagination[start]=${posts.length}&populate=*`
    );
...
export async function getServerSideProps() {
  ...
  const items = await fetchAPI(`/community-items?filters[biennials][slug][$ne]=biennial-2024&sort[0]=name&populate=*`);

	const totalItems = 
    await fetchAPI( `/community-items?filters[biennials][slug][$ne]=biennial-2024&sort[0]=name`
  );
