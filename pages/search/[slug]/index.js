...
const Search = ({ menus, global, items, search, numberOfPosts}) => {
  ...
  const getMorePosts = async () => {
    ...
    const communityItems = await fetchAPI(`/community-items?filters[$or][0][biennials][slug][$ne]=biennial-2024&filters[$or][1][biennials][slug][$null]=true&filters[slug][$contains]=${search}&pagination[start]=${posts.community.length}&populate=*`);
...
export async function getServerSideProps({params}) {
  ...
  const community = await fetchAPI(`/community-items?filters[$or][0][biennials][slug][$ne]=biennial-2024&filters[$or][1][biennials][slug][$null]=true&filters[slug][$contains]=${params.slug.replace(' ', '-')}&populate=*`);
