...
const Artists = ({ festival, menus, global, items, numberOfPosts, params }) => {
  ...
  const getMorePosts = async () => {
    const res = await fetchAPI(
      `/community-items?filters[$or][0][biennials][slug][$ne]=biennial-2024&filters[$or][1][biennials][slug][$null]=true&filters[biennials][slug][$eq]=${params.slug}&sort[0]=slug&pagination[start]=${posts.length}&populate=*`
    );
...
export async function getServerSideProps({params}) {
  ...
  const items = await fetchAPI(`/community-items?filters[$or][0][biennials][slug][$ne]=biennial-2024&filters[$or][1][biennials][slug][$null]=true&filters[biennials][slug][$eq]=${params.slug}&sort[0]=slug&populate=*`);
